...
1. 读 INPUT_DIR/TASKS.md
2. 读 INPUT_DIR/SUMMARY.md
3. 提取 @executor 或执行类任务
4. 写代码/解题/跑实验 → 产出放 OUTPUT_DIR/
5. 写 OUTPUT_DIR/execution_log.md
...
`skill_view(name='/absolute/path/...')` 会报错 "Non-relative patterns are unsupported"。
...
### 4. TASKS.md 格式可能变化
TASKS.md 的输出路径可能是 `OUTPUT/` 相对路径，也可能显式指定绝对路径。
